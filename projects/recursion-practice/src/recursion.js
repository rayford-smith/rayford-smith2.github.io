...
var range = function(x, y, output = []) {
  let mod = Math.sign(y-x);
  if (x+mod == y) return output;
  output.push(x+mod);
  return range(x+mod,y,output);
};
...
var reverseArr = function (array, retArr = []) {
  if (array.length == 0) return retArr;
  retArr.unshift(array[0]);
  return reverseArr(array.slice(1), retArr);
};
...
var rMap = function(array, callback, retArr = []) {
  if (array.length == 0) return retArr;
  retArr.push(callback(array[0]));
  return rMap(array.slice(1),callback,retArr);
};
...
var fibonacci = function(n,arr = [0]) {
  if (n == 0) return arr;
  if (arr.length == 1) arr.push(1);
  else arr.push(arr[0] + arr[1]);
...
// nthFibo(7); // 13
// nthFibo(3); // 2
var nthFibo = function(n, arr = [0]) {
  if (n < 0) return null;
  if (n == 0) return arr[arr.length-1];
  if (arr.length == 1) arr.push(1);
  else if (arr.length == 2) arr.push(1);
  else arr.push(arr[arr.length-2] + arr[arr.length-1]);
  return nthFibo(n-1,arr);
};
...
var compress = function(list, output = []) {
  ...
  if (list[0] != output[output.length-1] ) output.push(list[0]);
...
// alternateSign([-2,-7,8,3,-1,4]) // [2,-7,8,-3,1,-4]
var alternateSign = function(array, output = [], sign = 1) {
  if (array.length == 0) return output;
  if (Math.sign(sign) != Math.sign(array[0])) array[0] *= -1;
  output.push(array[0]);
  return alternateSign(array.slice(1),output, sign*-1);
...
var numToText = function(str, output = "", arr = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]) {
  if (str.length == 0) return output;
  if (isNaN(str[0])||str[0] === " ") output += str[0];
  else output+= arr[parseInt(str[0])];
